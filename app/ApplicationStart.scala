  ...
  def initialize(): Unit = {
    covidObservationDao.count.map {
      case 0 =>
        ...
        val datapoints =
          for (line <- Source.fromResource("covid_19_data.csv").getLines().drop(1))
            yield { // drop header line
              ...
              CovidObservation(
                ...
                LocalDateTime.parse(
                  cols(4),
                  DateTimeFormatter.ofPattern(lastUpdatePatterns)
                ),
        ...
        insertCount onComplete {
          case Success(n) =>
            logger.info(s"Seeded covid_observations with $n rows.")
