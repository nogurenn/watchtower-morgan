...
trait Tables {
  ...
  /** Entity class storing rows of table CovidObservations
    ...
    */
  case class CovidObservationsRow(
      id: Long,
  ...
  implicit def GetResultCovidObservationsRow(implicit
      e0: GR[Long],
      e1: GR[java.time.LocalDate],
      e2: GR[Option[String]],
      e3: GR[String],
      e4: GR[java.time.LocalDateTime],
      e5: GR[Int],
      e6: GR[Option[java.time.LocalDateTime]]
  ): GR[CovidObservationsRow] =
    GR { prs =>
      import prs._
      CovidObservationsRow.tupled(
        (
          <<[Long],
          <<[java.time.LocalDate],
          <<?[String],
  ...
  class CovidObservations(_tableTag: Tag)
      extends profile.api.Table[CovidObservationsRow](
        _tableTag,
        "covid_observations"
      ) {
    def * =
      (
        ...
        dead,
        recovered
      ) <> (CovidObservationsRow.tupled, CovidObservationsRow.unapply)
      ...
      (
        (
          ...
          { r =>
            ...
            _1.map(_ =>
              CovidObservationsRow.tupled(
                (_1.get, _2.get, _3, _4.get, _5.get, _6.get, _7.get, _8.get)
              )
            )
          },
          (_: Any) =>
    ...
    val observationDate: Rep[java.time.LocalDate] =
      column[java.time.LocalDate]("observation_date")

    /** Database column locality SqlType(varchar), Length(255,true), Default(None) */
    val locality: Rep[Option[String]] = column[Option[String]](
      "locality",
    ...
    val country: Rep[String] =
      column[String]("country", O.Length(64, varying = true))

    /** Database column last_update SqlType(timestamp) */
    val lastUpdate: Rep[java.time.LocalDateTime] =
      column[java.time.LocalDateTime]("last_update")
